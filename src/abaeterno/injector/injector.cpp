...
//		cout << "   " << tracefile << endl;
		//This is not precise at all once we don't know the compressing ratio 
		access_count = 6 * File::length(tracefile) / (uint64_t) sizeof(Memory::TraceMem);
		if (gz == 0)
		{
...
//		<< " read: "<< trace.isRead() << " - " << trace.getLength() << " bytes" << endl;
	...
	//cout << "Injecting at " << Mem->Name() << " values " << *trace << endl;
...
class Machine : public metric
{
	...
	uint64_t get_mem_accesses(void) 
	{
		uint64_t count=0;
		for(Mems::iterator i=mems.begin();i!=mems.end();i++)
			count+=(*i)->get_regscount();
		return count;
	}
...
void Machine::memory_inject_(Memory::lua& m, string tracefile)
{
	...
	//This access count is now wrong because compressing is being used for the tracefile.
	cout << "Memory accesses in file: " << im->get_regscount() << endl;   
		...
		else
		{
			if (injection_type == "memory")
			{
				...
				try
				{
					...
					uint64_t records= maxAccesses==0 ? Machine::get()->get_mem_accesses() : maxAccesses;
					...
					for(uint64_t qAcc=0; maxAccesses==0 || qAcc < maxAccesses; qAcc++)
					{
				    	...
				    	if( (act-init)%5==0 && act>next )
				    	{
				    		cout << "Time " << act - init << " : " << qAcc/1000000 << " M accesses. "
				   				 ...
				   				 << " Time left: " << (act-init)*records/qAcc -(act-init) << " sec."
				   				 << endl;
				    		next=act;
				    	}
					}
			...
			{
				if (injection_type == "disk")
				{
					cout << "Disk injection is not yet implemented. " << endl;
				}
				else
				{
					throw runtime_error("Invalid injection type '"+ injection_type + "'. Valid types are: 'cpu', 'memory' or 'disk'.");			
				}
			}
		}
