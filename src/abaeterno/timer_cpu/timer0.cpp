	...
	icache_fudge(p.get<double>("icache.fudge","1.0"))
{
	...
	trace_needs.types=!bpred_perfect; 
...
void Timer0::simulation(const Instruction* inst)
{
	...
	/* Branch predictor */
	if (!bpred_perfect && prev_inst && prev_inst->Type().is_branch())
	{
	...
	if(dcache)
	{
		...
		bool is_prefetch=inst->Type().is_prefetch();
		for(; il != el; ++il)
		{
			LOG(" -- LOAD:", *il);
...
void Timer0::full_warming(const Instruction* inst)
{
	...
	/* Branch predictor */		
	if (!bpred_perfect && prev_inst->Type().is_branch())
	{
