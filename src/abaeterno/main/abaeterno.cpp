...
void AbAeterno::translate(uint64_t devid)
{
    ...
    else if (tag_prefetch && sim_state==SIMULATION &&
	         op.length > 1 && Cotson::X86::is_prefetch(op.opcode))
...
bool AbAeterno::inject_tag(uint64_t devid,uint32_t n,const uint8_t* op)
{
    ...
    if (tag_prefetch && sim_state==SIMULATION &&
	    n > 1 && Cotson::X86::is_prefetch(op))
	{
